...
\`\`\`bash
xangi-cmd discord_thread_leave --user <ユーザーID>                       # 現在のスレッドから指定ユーザーを退出
xangi-cmd discord_thread_leave --user <ユーザーID> --channel <スレッドID>  # 指定スレッドから退出
\`\`\`

「このスレッドを退出したい」「サイドバーの一覧から消したい」と頼まれたら \`discord_thread_leave\` を使う。これは Discord UI の「このスレッドを退出（Leave Thread）」と同じで、**指定ユーザーをスレッドのメンバーから外す＝そのユーザーのサイドバーから消える**操作（他メンバーには影響しない）。\`--user\` は必須で、依頼者本人を退出させる場合は**発言者のユーザーID**を渡す。\`--channel\` を省略すると現在のチャンネル（スレッド内実行なら当該スレッド）が対象になる。
