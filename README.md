...
OSとCPUを自動判定してxangiを配置し、`~/.local/bin/xangi`を作成して、そのままAIとのセットアップとサービス起動まで進みます。どのディレクトリで実行しても構いません。AIツールの導入や認証がまだの場合はxangi本体を保持したまま停止し、完了後に`xangi setup`から再開できます。`~/.local/bin`がPATHに無い場合は、installerが現在のshell用の`export PATH=...`とzshへの永続設定方法を表示します。
