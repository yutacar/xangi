...
例: Codexを準備（`claude-code` / `cursor` / `grok` / `antigravity`も選択可能）
...
インストール後、新しいTerminalを開いてセットアップを開始

```bash
xangi setup
```

`xangi setup`を案内するAIの質問に従って、workspaceとWeb Chatの公開範囲を設定します。
...
最後の引数は`codex`、`claude-code`、`cursor`、`grok`、`antigravity`から選びます。インストール・認証状態だけを確認する場合は最後の引数を`check`にします。スクリプトは未導入の場合だけ各提供元の公式手順でインストールし、対話型の認証を開始します。Codexを選んだ場合はNode.jsとnpmが必要です。不足していれば、nvmの導入、Terminalの開き直し、`nvm install --lts`、Codexセットアップの再実行までを順番に表示して停止します。
...
OSとCPUを自動判定してxangiを配置し、`~/.local/bin/xangi`を作成します。`curl ... | bash`のpipe内ではAIの対話UIを起動せず、インストール完了後に通常のTerminalから`xangi setup`を実行するよう案内します。これはshellのpipeからCodexなどのTUIへ端末を引き継ぐ際のplatform差を避けるためです。どのディレクトリで実行しても構いません。`~/.local/bin`がPATHに無い場合はbashまたはzshの起動設定へ重複なく追加し、現在のshell用の`export PATH=...`も表示します。
...
installerを実行したTerminalでは親shellのPATHを変更できないため、最後に表示された`export PATH=...`を実行するか、新しいTerminalを開いてください。bashとzshの起動設定にはinstallerが`~/.local/bin`を追加します。
...
セットアップ中にDiscordの許可ユーザーIDや、Discord、Slack、LINE、Telegram、Notionのトークンが必要になったら、次の1コマンドでローカル設定画面を開きます。

```bash
xangi settings
```

値はAIとの会話やshell historyへ渡さず、OS別の専用設定領域へmode 0600で保存します。設定画面は`127.0.0.1`だけで一時的に開き、保存済みの値をブラウザへ返さず、保存後に終了します。
