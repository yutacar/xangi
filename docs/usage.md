...
`./bin/xangi service start|stop|restart|status` は、xangi の外側にある supervisor を操作する上位コマンドです。PM2 運用では、その clone の `.env` にある `XANGI_PROCESS_NAME` のプロセスを対象にします。
...
非対話実行は `agy --print-timeout <timeout> --output-format json -p ...` です。Agy CLI 1.1.2 の最終JSONから `status`、`response`、`conversation_id` を利用し、`conversation_id` を provider session として返します。`ANTIGRAVITY_PRINT_TIMEOUT`（デフォルト `5m`）で Agy 自身の print mode タイムアウトを設定できます。`AGENT_MODEL` が設定されていれば `--model`、provider session があれば `--conversation` を渡します。作業ディレクトリが設定されている場合は、子プロセスの cwd と同じ場所を `--add-dir .` で明示します。

`--output-format` を明確に未対応と報告する旧版 Agy は、同じ runner 内で旧プレーン出力モードへ一度だけフォールバックし、その後は旧モードをキャッシュします。timeout、認証、quota、無効なmodelなど通常の実行エラーでは再実行しません。
...
Antigravity CLI の真の逐次ストリーミング（`stream-json`）は未対応です。`runStream` でも最終応答を1回だけ通知します。
