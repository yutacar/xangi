...
- `message` event は通常メッセージだけを処理し、チャンネル名変更など `subtype` 付きの Slack システム通知は無視する
...
`/autoreply` / `/notify` / `/threadmode` は `settings.json`、`/respondtobots` / `/backend` / `/llmmode` は `.env` 書き戻しを使う。`.env` 側の「設定の持続」は **2 層構造** で動作する。混同しがちなので明示的に整理する:
