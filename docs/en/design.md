...
- `packaging/bootstrap.sh` is published as the shared `install.sh` entry point for every supported operating system. It detects Darwin / Linux and arm64 / x64, then dispatches to the target installer in the same GitHub Release. WSL2 follows the Linux path. When invoked through a pipe, it reconnects interactive setup stdin to the controlling `/dev/tty`; without a controlling terminal, it defers setup and service activation
